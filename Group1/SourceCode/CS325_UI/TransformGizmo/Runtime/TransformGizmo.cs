...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		//Added by group 1
		GameObject _toCopy;
		public GameObject _sceneParent;
		bool _cut = false;
		public GameObject _radioTransform;
		public bool _translateSnap = false;
		public bool _rotateSnap = false;
		public bool _scaleSnap = false;

		public List<float> _translateSnapSizes;
		public List<float> _rotateSnapSizes;
		public List<float> _scaleSnapSizes;
		...
		void Update()
		{
			...
			//Written by Group 1
			//Checks for the command execution in the respective commands
			CutSelected();
			CopySelected();
			PasteSelected();
			DeleteSelected();
		...
		public void SetTransformType(int index)
        {
			transformType = (TransformType)index;
		}

		public void ToggleSnap(int index)
        {
			if(index == 0)
			{
				_translateSnap = !_translateSnap;
			}
			else if(index == 1)
            {
				_rotateSnap = !_rotateSnap;
			}
			else
			{
				_scaleSnap = !_scaleSnap;
			}
        }
		...
		void SetSpaceAndType()
		{
			...
			if (Input.GetKey(ActionKey))
			{
				if (Input.GetKeyDown(SetMoveType))
				{
					...
					_radioTransform.transform.GetChild(0).GetComponent<Toggle>().SetIsOnWithoutNotify(true);
				}
				else if (Input.GetKeyDown(SetRotateType))
				{
					transformType = TransformType.Rotate;
					_radioTransform.transform.GetChild(1).GetComponent<Toggle>().SetIsOnWithoutNotify(true);
				}
				else if (Input.GetKeyDown(SetScaleType))
				{
					transformType = TransformType.Scale;
					_radioTransform.transform.GetChild(2).GetComponent<Toggle>().SetIsOnWithoutNotify(true);
				}
		...
		//Added by group 1, for use with buttons and keys
		public void CutSelected(bool skip = false)
		{
			if (mainTargetRoot != null)
			{
				if (Input.GetKey(ActionKey) || skip)
				{
					if (Input.GetKeyDown(KeyCode.X) || skip)
					{
						_toCopy = mainTargetRoot.gameObject;
						_cut = true;
					}
				}
			}
		...
		//Added by group 1, for use with buttons and keys
		public void CopySelected(bool skip = false)
		{
			if (mainTargetRoot != null)
			{
				if (Input.GetKey(ActionKey) || skip)
				{
					if(Input.GetKeyDown(KeyCode.C) || skip)
                    {
						_toCopy = mainTargetRoot.gameObject;
                    }
				}
		...
		//Added by group 1, for use with buttons and keys
		public void PasteSelected(bool skip = false)
		{
			if (_toCopy != null)
			{
				if (Input.GetKey(ActionKey) || skip)
				{
					if (Input.GetKeyDown(KeyCode.V) || skip)
					{
						if (_cut)
						{
							Destroy(mainTargetRoot.gameObject);
							_cut = false;
						}

						ClearTargets();
						GameObject go = Instantiate(_toCopy);
						go.transform.parent = _sceneParent.transform;
					}
				}
			}
		}
		...
		//Added by group 1, for use with buttons and keys
		public void DeleteSelected(bool skip = false)
		{
			if (mainTargetRoot != null)
			{
				if (Input.GetKeyDown(KeyCode.Delete) || skip)
				{
					Destroy(mainTargetRoot.gameObject);
				}
			}
		...
		public void SetTranslateSnapSize(int index)
        {
			movementSnap = _translateSnapSizes[index];
        }

		public void SetRotationSnapSize(int index)
		{
			rotationSnap = _rotateSnapSizes[index];
		}

		public void SetScaleSnapSize(int index)
		{
			scaleSnap = _scaleSnapSizes[index];
		}
			...
			while(!Input.GetMouseButtonUp(0))
			{
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					if(transType == TransformType.Move)
					{
						...
						if((isSnapping || _translateSnap) && movementSnap > 0)
						{
					...
					else if(transType == TransformType.Scale)
					{
						...
						if((isSnapping || _scaleSnap) && scaleSnap > 0)
						{
					...
					else if(transType == TransformType.Rotate)
					{
						...
						if((isSnapping || _rotateSnap) && rotationSnap > 0)
						{
		...
		bool mouseOverViewport(Camera main_cam)
		{
			...
			return main_cam.rect.Contains(main_mou);
		}

		void GetTarget()
		{
			//Just this if statement added by group 1
			//Deals with the bug for smaller viewport unable to click button without interfering with functionality
			if (mouseOverViewport(this.GetComponent<Camera>()))
			{
				if (nearAxis == Axis.None && Input.GetMouseButtonDown(0))
				{
					...
					if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo, Mathf.Infinity, selectionMask))
					{
						...
						if (isAdding && !IfContains(target))
						{
							AddTarget(target);
						}
						else if (isRemoving && IfContains(target))
						{
							RemoveTarget(target);
						}
		...
		public bool IfContains(Transform target)
        {
			if (targetRoots.ContainsKey(target)) return true;
